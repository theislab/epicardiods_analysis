...
if __name__ == '__main__':
    ...
    # This needs to be uncommented if you only want RNA cells that have an ATAC match
    
    #g1_idx = g1_idx[~any_nans].astype(int)
    #g2_idx = g2_idx[~any_nans].astype(int)
    matches = pd.DataFrame({
        group1_name: group1.index[g1_idx.astype(int)],
        group2_name: [group2.index[idx.astype(int)] if not np.isnan(idx) else "None" for idx in g2_idx] 
    })
