...
class ExperimentWrapper:
    ...
    def init_dataset(self, dataset, batch):
        ...
        if dataset == "epicardiods":
            data_path = os.path.join("/storage/groups/ml01/workspace/laura.martens/moretti_colab/archr_new", "Epicardiods", "Anndata")
            self.adata = sc.read(os.path.join(data_path, "adata_filtered.h5ad"))
            patac.data.reads_to_fragments(self.adata, key_added="counts")
            self.adata.X = (self.adata.X > 0).astype(float)
